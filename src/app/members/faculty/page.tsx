...
import { Users, ShieldCheck, Linkedin, Github, GraduationCap } from "lucide-react";
...
const FacultyMemberCard = ({ member }: { member: Member }) => (
...
export default function FacultyPage() {
  ...
  useEffect(() => {
    async function fetchMembers() {
      setIsLoading(true);
      const fetchedMembers = await getMembers('Faculty');
        ...
        <div className="container mx-auto px-4 py-20 sm:px-6 lg:px-8">
          <div className="text-center space-y-8">
            <div className="inline-flex items-center px-6 py-3 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-sm font-medium">
                ...
                <span className="text-white font-semibold">Faculty Advisors</span>
            </div>
            <div className="space-y-4">
              <h1 className="text-5xl md:text-7xl font-bold bg-gradient-to-r from-white via-cyan-200 to-blue-200 bg-clip-text text-transparent leading-tight font-headline">
                Our Mentors
              </h1>
              <div className="w-24 h-1 bg-gradient-to-r from-cyan-500 to-blue-500 mx-auto rounded-full"></div>
            </div>
            <p className="text-lg md:text-xl text-slate-300 max-w-3xl mx-auto leading-relaxed font-light">
              Meet the experienced faculty members who guide our club, provide invaluable mentorship, and support our initiatives.
            </p>
          </div>
        </div>
      </div>
        ...
        {isLoading ? (
            <LoadingSkeletons />
             ...
             <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">No faculty members found</h3>
                <p className="text-gray-400">Faculty members will be displayed here once they are added.</p>
            </div>
        )}
      </div>
    </div>
